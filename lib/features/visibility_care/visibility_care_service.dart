...
class VisibilityCareService {
  VisibilityCareService({
  ...
  Future<List<VisibilityCareSnapshot>> fetchRecent(String vehicleId) async {
    try {
      final rows = await _client
          .from('visibility_care_checks')
          .select(
      ...
      return (rows as List<dynamic>)
          .map(
            (row) => VisibilityCareSnapshot.fromMap(
              Map<String, dynamic>.from(row as Map),
            ),
          )
          .toList(growable: false);
    } catch (_) {
      throw const VisibilityCareException(
  ...
  Future<void> save({
    ...
    required VisibilityCareAssessment assessment,
  }) async {
    profile.validate();
    try {
      await _client.from('visibility_care_checks').insert({
        ...profile.toMap(),
        ...
        'calculator_version': 'visibility-care-v1',
      });
    } catch (_) {
      throw const VisibilityCareException(
        'Impossible d’enregistrer ce contrôle de visibilité.',
      );
    }
  }
}

class VisibilityCareException implements Exception {
  const VisibilityCareException(this.message);
